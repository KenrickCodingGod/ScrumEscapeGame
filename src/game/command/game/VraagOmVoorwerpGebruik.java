...
public class VraagOmVoorwerpGebruik implements Command {
    ...
    public void voerUit() {
        ...
        switch (input) {
            case "gebruik zwaard" -> {
                if (zwaard != null) ui.toon(zwaard.attack(monster));
                else ui.toon("❌ Geen zwaard beschikbaar.");
            }
            case "gebruik boek" -> {
                if (boek != null) ui.toon("📖 " + boek.showMessage());
                else ui.toon("❌ Geen boek beschikbaar.");
            }
            case "" -> {}
            default -> new VraagOmVoorwerpGebruik(kamer, monster, ui).voerUit(); // herhaal
        }
    }
}
